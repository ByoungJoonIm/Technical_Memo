...
### 교착상태
- 교착 상태 : 두 개 이상의 프로세스가 서로 점유한 상대방의 자원을 기다리며 결과적으로 아무것도 완료하지 못하는 상태
- 발생 조건
  - 상호배제 : 프로세스들이 필요로 하는 자원에 대한 배타적인 통제권을 요구
  - 점유대기 : 프로세스가 할당된 자원을 가진 상태에서 다른 자원을 기다림
  - 비선점 : 프로세스가 어떤 자원의 사용을 끝낼 때까지 그 자원을 뺏을 수 없음
  - 환형대기 : 각 프로세스는 순환적으로 다음 프로세스가 요구하는 자운을 가짐
- 해결 방법
  - 회피 : 교착상태 발생 조건 네가지중 1가지를 제거
  - 예방 : 교착상태의 발생 조건을 없애지 않고 발생하지 않도록 알고리즘을 적용. 예를 들어 은행원 알고리즘
  - 회복 : 교착상태가 발생하는 것을 막지 않고, 발생 후 문제를 해결
  - 무시 : 교착상태 해결로 인한 비용이 더 큰 경우 교착상태를 무시
